...
try {
  ...
  console.error(
    "profileCoach: LangChain/Groq/zod dependencies unavailable, AI profile coaching disabled.",
    loadErr.message
  );
}
...
if (apiKey && ChatGroq) {
  try {
    ...
    console.error(
      "profileCoach: Failed to initialize ChatGroq model, AI profile coaching disabled.",
      initErr.message
    );
    model = null;
  }
...
if (apiKey && ChatGroq) {
  try {
    ...
    console.error(
      "profileCoach: Failed to initialize chat ChatGroq model, AI Coach chat disabled.",
      initErr.message
    );
    chatModel = null;
  }
    ...
    outputParser = StructuredOutputParser.fromZodSchema(
      z.object({
        overallScore: z
          ...
          .min(0)
          .max(100)
          .describe("Overall profile strength score from 0 to 100"),

        summary: z
          .string()
          .describe(
            "A short 1-3 sentence overall summary of the profile's current state"
          ),

        strengths: z
          .array(z.string())
          .describe(
            "2-5 short bullet points describing what is already strong about the profile"
          ),

        missingFields: z
          .array(z.string())
          .describe(
            "Short list of important profile fields that are empty or missing"
          ),

        improvements: z
          .array(z.string())
          .describe(
            "3-6 short, concrete, actionable improvement suggestions"
          ),

        suggestedSkills: z
          .array(z.string())
          .describe(
            "2-6 complementary in-demand skills the developer could add"
          ),

        betterBio: z
          .string()
          .describe(
            "A rewritten, improved version of the developer's 'about' bio, 2-4 sentences"
          ),
      })
    );
  } catch (parserErr) {
    console.error(
      "profileCoach: Failed to build StructuredOutputParser, AI profile coaching disabled.",
      parserErr.message
    );
    outputParser = null;
  }
...
if (ChatPromptTemplate && outputParser) {
  try {
    ...
    console.error(
      "profileCoach: Failed to build ChatPromptTemplate, AI profile coaching disabled.",
      promptErr.message
    );
    promptTemplate = null;
  }
...
if (RunnableSequence && promptTemplate && model && outputParser) {
  try {
    ...
    console.error(
      "profileCoach: Failed to build RunnableSequence, AI profile coaching disabled.",
      chainErr.message
    );
    chain = null;
  }
...
function computeMissingFields(user) {
  ...
  if (!user.about || user.about === "This is a default bio")
...
function buildDefaultStructuredResponse(user) {
  ...
  return {
    overallScore:
      typeof user.profileStrength === "number"
        ? user.profileStrength
        : 0,

    summary:
      "We couldn't generate a fresh AI analysis right now, but here's a quick automated check of your profile.",

    strengths:
      Array.isArray(user.skills) && user.skills.length > 0
        ? [
            `You've listed ${user.skills.length} skill${
    ...
    missingFields:
      missingFields.length > 0
        ? missingFields
        : ["No obviously missing fields detected."],
...
async function generateProfileSuggestions(user) {
  ...
  try {
    ...
    console.error(
      "profileCoach: AI profile suggestion generation failed.",
      error.message
    );
    return buildDefaultStructuredResponse(user);
  }
...
function buildChatSystemPrompt(user, suggestions) {
  const profileContext =
    ...
    `- Projects: ${
      Array.isArray(user.projects) && user.projects.length
        ? `${user.projects.length} project(s) listed`
        : "No projects listed yet"
    }\n` +
    `- College/Degree: ${user.college || "Not specified"} (${
    ...
    `- Connected Assets: GitHub: ${
      user.github ? "Linked" : "Missing"
    } | LinkedIn: ${
      user.linkedin ? "Linked" : "Missing"
    } | Portfolio: ${
      user.portfolio ? "Linked" : "Missing"
    } | Resume: ${
  ...
  const reportContext = suggestions
    ? "\n\nPreviously Generated Coaching Report (for context, don't just repeat it):\n" +
      ...
      `- Strengths: ${
        Array.isArray(suggestions.strengths)
          ? suggestions.strengths.join(" | ")
          : "None"
      }\n` +
      `- Missing Fields: ${
        Array.isArray(suggestions.missingFields)
          ? suggestions.missingFields.join(", ")
          : "None"
      }\n` +
      `- Improvements: ${
        Array.isArray(suggestions.improvements)
          ? suggestions.improvements.join(" | ")
          : "None"
      }\n` +
      `- Suggested Skills: ${
        Array.isArray(suggestions.suggestedSkills)
          ? suggestions.suggestedSkills.join(", ")
          : "None"
      }\n` +
...
async function generateCoachChatReply(
  user,
  suggestions,
  history,
  message
  ...
  try {
    ...
    console.error(
      "profileCoach: AI Coach chat reply failed.",
      error.message
    );
    return null;
  }
